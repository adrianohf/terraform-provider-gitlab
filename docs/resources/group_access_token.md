...
- `access_level` (String) The access level for the group access token. Valid values are: `no one`, `minimal`, `guest`, `planner`, `reporter`, `security_manager`, `developer`, `maintainer`, `owner`. Default is `maintainer`.
